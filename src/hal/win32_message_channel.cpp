...
class Win32MessageChannel : public MessageChannelInterface {
public:
    static constexpr size_t MAX_CAPACITY = DEFAULT_CAPACITY;

    explicit Win32MessageChannel(size_t capacity)
...
MessageChannelHandle create_platform_message_channel(size_t capacity) {
    ...
    return MessageChannelHandle(new Win32MessageChannel(capacity), MessageChannelHandleDeleter{});
}
