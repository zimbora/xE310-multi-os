...
NetworkLte::NetworkLte(xE310& modem,
                       const NetworkLteConfig& config,
...
const std::string& NetworkLte::iccid() const {
    return modemInfo.iccid;
}
const std::string& NetworkLte::imsi() const {
...
const std::vector<Operator>& NetworkLte::available_operators() const {
    return operatorList;
}
...
void NetworkLte::new_connection(uint8_t conn_id,
...
bool NetworkLte::server_connect(uint8_t conn_id,
                                const std::string& protocol,
                                const std::string& ip,
                                const uint16_t port) {
    ...
    NETWORK_LOG_DBG("Attempting to connect to server with CID %d, protocol %s, IP %s, port %d",
                    conn_id,
                    protocol.c_str(),
                    ip.c_str(),
                    port);
    if (state_ == NetworkLteState::data_ready) {
        ...
        NETWORK_LOG_DBG("Checking current connection state for CID %d before connecting to server",
                        conn_id);
        ...
        if (serverInfo[conn_id - 1].state == ServerState::connected) {
            NETWORK_LOG_INF("Already connected to a server, cannot connect to a different one "
                            "without disconnecting first");
            return true; // already connected to a server, cannot connect to a different one without
                         // disconnecting first
        }
        if (serverInfo[conn_id - 1].state == ServerState::suspended) {
            ...
            if (status != ModemStatus::ok) {
                ...
                NETWORK_LOG_INF(
                    "Successfully closed connection to server at %s:%d", ip.c_str(), port);
            }
        }
        /*
        ...
        */
        if (serverInfo[conn_id - 1].state == ServerState::dns_resolving ||
            ...
            NETWORK_LOG_ERR("Already in the process of connecting to a server, cannot connect to a "
                            "different one without disconnecting first");
            return false; // already in the process of connecting to a server, cannot connect to a
                          // different one without disconnecting first
        }
        ...
        if (protocol == "UDP") {
            ...
            NETWORK_LOG_WRN(
                "TCP protocol is not currently supported, cannot connect to server at %s:%d",
                ip.c_str(),
                port);
            return false; // TCP not supported, cannot connect
        } else {
            NETWORK_LOG_ERR(
                "Unknown protocol for server connection, cannot connect to server at %s:%d",
                ip.c_str(),
                port);
...
bool NetworkLte::server_disconnect(uint8_t conn_id) {
    ...
    if (serverInfo[conn_id - 1].protocol == "UDP") {
        NETWORK_LOG_INF("Attempting to disconnect from UDP server at %s:%d",
                        serverInfo[conn_id - 1].address.c_str(),
        ...
        NETWORK_LOG_WRN(
            "TCP protocol is not currently supported, cannot disconnect from server at %s:%d",
            serverInfo[conn_id - 1].address.c_str(),
            serverInfo[conn_id - 1].port);
...
bool NetworkLte::force_PSM() {
    ...
    // It will try to connect to each availale operator and enter PSM mode, if the modem and network
    // support it. It will stay in PSM mode for a short time and then exit, to demonstrate the PSM
    // enter and exit flows and events. This is meant to be used for testing purposes, to force the
    // modem into PSM mode and trigger the corresponding events.
    ...
    for (const auto& op : availableOperators) {
        NETWORK_LOG_INF("Trying to register to operator %s with radio tech %d to force PSM mode",
                        op.long_name.c_str(),
                        static_cast<int>(op.act));
        if (op.act == RadioTech::cat_m1 || op.act == RadioTech::nb_iot)
            NETWORK_LOG_INF("Operator %s supports tech with PSM, trying to register to it",
                            op.long_name.c_str());
        ...
        if (reg_info.stat == RegStatus::registered_home ||
            reg_info.stat == RegStatus::registered_roaming) {
            ...
            NETWORK_LOG_INF("PSM status for operator %s: status=%d, mode=%d, t3412=%u, t3324=%u "
                            "psm_version=%d, psm_threshold=%u",
                            op.long_name.c_str(),
                            tCpsmsStatus.status,
                            static_cast<int>(tCpsmsStatus.mode),
                            tCpsmsStatus.t3412,
                            tCpsmsStatus.t3324,
                            static_cast<int>(tCpsmsStatus.psm_version),
                            tCpsmsStatus.psm_threshold);
            if (tCpsmsStatus.mode == PsmMode::disable) {
                ...
                NETWORK_LOG_WRN(
                    "PSM timers are not configured for this operator, cannot enter PSM mode");
                continue; // try next operator
            } else if (tCpsmsStatus.t3412 == lteConfig.psm_t3412 &&
                       tCpsmsStatus.t3324 == lteConfig.psm_t3324) {
                NETWORK_LOG_INF("PSM timers were accepted by network for this operator");
                psmModeAttached = true;
                break;
            } else {
                NETWORK_LOG_WRN("PSM timers were not accepted by network for this operator, but "
                                "different timers were granted");
                psmModeAttached = true;
                break;
            }
        } else {
            NETWORK_LOG_WRN("Failed to register to operator %s, cannot enter PSM mode",
                            op.long_name.c_str());
            continue; // try next operator
        }
    }
    ...
    if (psmModeAttached)
        ...
        change_state(NetworkLteState::idle_mode); // if we couldn't enter PSM mode with any
                                                  // operator, go back to idle mode
...
bool NetworkLte::exit_transparent_mode() {
    ...
    go_to_state(NetworkLteState::idle_mode); // after exiting transparent mode, go back to idle mode
                                             // to restart normal flow
    if (state_ != NetworkLteState::transparent_mode) {
...
bool NetworkLte::send_at_command(const std::string& command,
                                 std::string& response,
                                 uint32_t timeout_ms) {
    // send command
    if (state_ != NetworkLteState::transparent_mode) {
...
bool NetworkLte::update_modem(const std::string& firmware_url) {
    ...
    if (state_ != NetworkLteState::modem_fota) {
        ...
        fColdBoot = true; // after FOTA, we can consider the next boot as the first one to re-apply
                          // configuration
        go_to_state(NetworkLteState::setup_mode); // after FOTA, go back to setup mode to re-apply
                                                  // configuration and re-attach to network
        return true;
    }
}
...
NetworkLteState NetworkLte::loop(NetworkLteState target_state) {
    ...
    case NetworkLteEvent::psm_exit:
        ...
        if (prev_state_ == NetworkLteState::data_ready)
            change_state(
                NetworkLteState::data_ready); // assume we were in sleep mode with an active
                                              // connection, we can go directly to data ready state
        else {
            change_state(
                NetworkLteState::idle_mode); // otherwise go back to idle and restart attach flow
        }
        break;
    ...
    case NetworkLteEvent::network_attached:
        ...
        change_state(
            NetworkLteState::pdp_context_closed); // after network attach, we can assume any
                                                  // previous PDP context is now deactivated, so we
                                                  // can go to context deactivated state and trigger
                                                  // PDP activation flow from there
        break;
    case NetworkLteEvent::context_closed: {
        ...
        change_state(
            NetworkLteState::network_detached); // after context is closed, we can assume we are
                                                // detached from network, so we can go to detached
                                                // state and restart attach flow from there
    } break;
    case NetworkLteEvent::context_opened:
        NETWORK_LOG_INF("Network PDP context opened, IP address assigned: %s",
                        networkInfo.ip_address.c_str());
        change_state(NetworkLteState::data_ready);
        break;
    ...
    case NetworkLteEvent::timeout: {
        if (state_ == NetworkLteState::data_ready) {
            call_action(
                ModemAction::enter_sleep); // if we are in data ready state and we receive a timeout
                                           // event, we can assume the connection is lost, so we can
                                           // trigger radio shutdown to restart the attach flow
        } else if (state_ == NetworkLteState::transparent_mode) {
            call_action(
                ModemAction::leave_transparent_mode); // if we are in transparent mode and we
                                                      // receive a timeout event, we can assume the
                                                      // connection is lost, so we can trigger exit
                                                      // transparent mode to restart the normal flow
                                                      // and eventually go back to data ready if the
    ...
    case NetworkLteEvent::at_command_no_response:
        call_action(
            ModemAction::check_responsiveness); // if we receive an AT command error, we can check
                                                // if the modem is still responsive, if not, we can
                                                // trigger a reboot to try to recover
        break;
    default:
...
bool NetworkLte::go_to_state(NetworkLteState target_state) {
    ...
    while (state_ != target_state) {
        // This function generates the necessary sequence of events to transition from the current
        // state to the target state.
        // For simplicity, we will just call loop() in a loop until we reach the target state,
        // relying on the fact that loop() will process events and update the state accordingly. In
        // a more complex implementation, we could have a predefined map of required events for each
        // state transition to speed this up and avoid unnecessary iterations.
        switch (state_) {
        ...
        case NetworkLteState::transparent_mode:
            if(target_state == NetworkLteState::data_ready || target_state ==
        NetworkLteState::idle_mode){ call_action(ModemAction::leave_transparent_mode); // trigger
        exit transparent mode to go back to idle mode and restart normal flow, which should lead us
        back to data ready if the connection is still active
            }
            break;
        */
        default:
            ...
            break; // for other states, we will rely on the normal event flow to transition to the
                   // target state
        }

        loop(target_state); // process events and update state until we reach the target state
        // In a real implementation, we might want to add a timeout here to avoid infinite loops in
        // case of unexpected conditions. if timeout reached:
        //     NETWORK_LOG_ERR("Timeout while trying to transition from %s to %s",
        //     state_to_str(state_), state_to_str(target_state)); return false;
        switch (event_) {
        case NetworkLteEvent::network_error:
        case NetworkLteEvent::attach_error:
        case NetworkLteEvent::context_error:
            NETWORK_LOG_ERR("Error event received while trying to transition to target state, "
                            "aborting transition");
            return false; // if we receive an error event during the transition, we can consider the
                          // transition failed and return false
        default:
...
void NetworkLte::execute_actions() {
    ...
    if (action != ModemAction::none)
        NETWORK_LOG_DBG("Executing action: %s", action_to_str(action));
    switch (action) {
    case ModemAction::reboot: {
        ...
        if (check_status_response(status)) {
            ...
            call_action(ModemAction::power_on); // after reboot, check responsiveness to continue
                                                // with the flow
        }
    } break;
    case ModemAction::check_responsiveness: {
        ...
        while (true) {
            ...
            if (status != ModemStatus::ok) {
                ...
                // call_action(ModemAction::reboot); // try rebooting the modem to recover
                // responsiveness
            } else {
                NETWORK_LOG_INF("Modem is responsive");
                change_state(NetworkLteState::idle_mode); // if modem is responsive, we can go to
                                                          // idle mode and continue with the flow
                call_action(
                    ModemAction::query_network_status); // trigger radio setup to start attach flow
                break;
            }
    ...
    case ModemAction::factory_reset: {
        ...
        if(status == ModemStatus::ok){
            change_state(NetworkLteState::rebooting); // after factory reset, the modem will reboot,
        so we can consider it in rebooting state and wait for it to be responsive again before
    ...
    case ModemAction::enter_sleep: {
        // check if modem is in PSM modem by reading GPIO state or PSM status, if not, we can
        // trigger radio shutdown to restart the attach flow and eventually enter PSM if it is
        // enabled in the configuration
        call_action(ModemAction::switch_off_radio); // if PSM is not enabled, we can trigger radio
                                                    // shutdown to restart the attach flow
    } break;
    case ModemAction::wake_up: {
        // drive GPIO0 + CFUN=1, Warm boot
        if (prev_state_ == NetworkLteState::data_ready) {
            NETWORK_LOG_INF("Waking up from sleep mode, previous state was data ready, trying to "
                            "go back to data ready state");
            change_state(NetworkLteState::data_ready); // assuming we were in sleep mode with an
                                                       // active connection, we can try to go back
                                                       // to data ready state directly
            call_action(ModemAction::query_network_status);
        } else {
            NETWORK_LOG_INF("Waking up from sleep mode, previous state was not data ready, going "
                            "to idle mode to restart attach flow");
            fWarmBoot = true;
            change_state(
                NetworkLteState::idle_mode); // after waking up, we can go back to idle mode and
                                             // check network status to continue with the flow
            call_action(ModemAction::setup_radio); // trigger attach flow in idle mode
        }
    } break;
    ...
    case ModemAction::query_network_status: {
        ...
        if (regInfo.stat == RegStatus::denied || regInfo.stat == RegStatus::not_registered ||
            ...
            call_action(ModemAction::query_pdp_context); // trigger PDP activation flow in context
                                                         // closed state
        ...
        if (nAttachRetries == 0) {
            ...
            if (status == ModemStatus::ok) {
                if (current_tech != lteConfig.default_iot_tech) {
                    NETWORK_LOG_WRN("Current IoT technology %d is different from default config "
                                    "%d, changing it to default config and rebooting to apply",
                                    static_cast<int>(current_tech),
                                    static_cast<int>(lteConfig.default_iot_tech));
                    fChangeRAT = true;
                }
            }
            // check default bands
            ...
            if (status == ModemStatus::ok) {
                if (bands.lte_mask != lteConfig.default_lte_bands) {
                    NETWORK_LOG_WRN("Current LTE bands %llu are different from default config "
                                    "%llu, changing it to default config and rebooting to apply",
            ...
            // if any diferent set it to default config and reboot to apply, then start attach flow
            // in idle mode
            if (fChangeRAT || fChangeBands) {
                call_action(ModemAction::setup_radio); // if we need to change either RAT or bands,
                                                       // we can just reboot once and apply both
            ...
            if (status == ModemStatus::ok) {
                if (regInfo.apn != lteConfig.default_apn) {
                    NETWORK_LOG_WRN("Current APN %s is different from default config %s, changing "
                                    "it to default config",
                                    regInfo.apn.c_str(),
                                    lteConfig.default_apn.c_str());
            ...
            NETWORK_LOG_INF("Retrying network attach with fallback configuration, attempt %d",
                            nAttachRetries);
            ...
            if (status == ModemStatus::ok) {
                if (current_tech != lteConfig.fallback_iot_tech) {
                    NETWORK_LOG_WRN("Current IoT technology %d is different from fallback config "
                                    "%d, changing it to fallback config and rebooting to apply",
                                    static_cast<int>(current_tech),
                                    static_cast<int>(lteConfig.fallback_iot_tech));
                    fChangeRAT = true;
                }
            }
            ...
            if (status == ModemStatus::ok) {
                if (bands.lte_mask != lteConfig.fallback_lte_bands) {
                    NETWORK_LOG_WRN("Current LTE bands %llu are different from fallback config "
                                    "%llu, changing it to fallback config and rebooting to apply",
                                    static_cast<unsigned long long>(bands.lte_mask),
            ...
            // if any diferent set it to fallback config and reboot to apply, then start attach flow
            // in idle mode
            if (fChangeRAT || fChangeBands) {
                call_action(ModemAction::setup_radio); // if we need to change either RAT or bands,
                                                       // we can just reboot once and apply both
            ...
            if (status == ModemStatus::ok) {
                if (regInfo.apn != lteConfig.fallback_apn) {
                    NETWORK_LOG_WRN("Current APN %s is different from fallback config %s, changing "
                                    "it to fallback config",
                                    regInfo.apn.c_str(),
                                    lteConfig.fallback_apn.c_str());
            ...
            on_event(NetworkLteEvent::attach_error); // flag attach error to trigger power off in
                                                     // the next step
        }
        nAttachRetries++;
    } break;
    case ModemAction::query_pdp_context: {
        ...
        if (status != ModemStatus::ok) {
            ...
            change_state(
                NetworkLteState::pdp_context_closed); // if context is inactive, we can consider it
                                                      // closed and trigger PDP activation flow
            ...
            if (status != ModemStatus::ok) {
                ...
                change_state(NetworkLteState::data_ready); // already have IP, we can go directly to
                                                           // data ready state
            }
        }
    } break;
    ...
    case ModemAction::send_data: {
        if (state_ != NetworkLteState::data_ready) {
            NETWORK_LOG_WRN("Ignoring send_data action: state=%d (requires data_ready)",
        ...
        for (uint8_t id = 1; id <= MAX_SERVER_CONNECTIONS; ++id) {
            ...
            if (serverInfo[id - 1].state != ServerState::connected) {
                NETWORK_LOG_WRN("conn_id %d not connected, state: %d skipping TX drain",
                                id,
            ...
            while (message_queue_->tx_pop(id, msg) == QueueError::ok) {
                NETWORK_LOG_INF(
                    "id: %d, sending message of length %d", id, static_cast<int>(msg.data.size()));
    ...
    case ModemAction::read_data:
        // for testing, we can just read data from the modem and print it, in real implementation,
        // we would pass it to a buffer
        {
            std::vector<uint8_t> buf;
    ...
    case ModemAction::data_complete:
        // signal server that we are going offline for a while (e.g. going to sleep mode) so it
        // should not expect data from us and we can close the connection gracefully
        break;
    case ModemAction::enter_transparent_mode: {
        // switch modem to transparent mode, in this mode we can just send data to the modem and it
        // will be sent directly without needing to use UDP send/receive functions, this is useful
        // for low latency applications where we want to minimize the time between sending data and
        // it being sent over the network do any task needed to switch to transparent mode (e.g.
        // close existing PDP context, set up new PDP context with transparent mode settings, etc.)
        ...
        modem_.set_psm_urc(false); // disable PSM URCs in transparent mode to avoid interfering with
                                   // raw data reception
        modem_.set_registration_urc(false); // disable registration URCs in transparent mode to
                                            // avoid interfering with raw data reception
        modem_.set_pdp_urc(false); // disable PDP URCs in transparent mode to avoid interfering with
                                   // raw data reception
        NETWORK_LOG_INF("Modem in transparent mode (ready to receive AT commands)");
    } break;

    case ModemAction::leave_transparent_mode: {
        // switch modem to normal mode, in this mode we use UDP send/receive functions for data
        // transmission do any task needed to switch to normal mode (e.g. close existing PDP
        // context, set up new PDP context with normal mode settings, etc.)
        NETWORK_LOG_INF("Modem leaving transparent mode (ready to receive AT commands)");
        modem_.set_psm_urc(true);          // enable PSM URCs in normal mode
        modem_.set_registration_urc(true); // enable registration URCs in normal mode
        modem_.set_pdp_urc(true);          // enable PDP URCs in normal mode
        change_state(
            NetworkLteState::idle_mode); // after leaving transparent mode, we can go back to idle
                                         // mode and restart attach flow to ensure we are properly
                                         // connected before sending/receiving data
        call_action(
            ModemAction::query_network_status); // trigger attach flow to ensure we are properly
                                                // connected before sending/receiving data
    } break;
    }
}
...
// actions to perform on state change, such as starting/stopping timers, logging state transitions,
// etc.
void NetworkLte::change_state(NetworkLteState new_state) {
    ...
    // stop any timers that are running for the previous state, and log elapsed time for
    // debugging/analytics purposes
    switch (prev_state_) {
    ...
    // init timers for states that require timeouts to trigger retries or error handling if we get
    // stuck in those states for too long (e.g. if we are attaching to the network but it is taking
    // too long, we can trigger a retry or fallback to a different configuration)
    switch (state_) {
    case NetworkLteState::network_attaching:
    ...
    case NetworkLteState::data_ready: {
        ...
        // modem_.get_iot_tech(regInfo.act, networkInfo.gsm_priority); // update IoT tech in info
        // struct after applying it to modem modem_.get_registration_status(regInfo,
        // RadioTech::gsm); // update registration info in info struct after applying it to modem
        modem_.get_signal_quality(
            signalQuality); // update signal quality in info struct after applying it to modem
        modem_.get_operator(
            regInfo
                .operator_name); // update operator name in info struct after applying it to modem
        modem_.get_apn(lteConfig.cid,
                       regInfo.apn); // parse +CGDCONT? into regInfo.apn and regInfo.ip_address
        // modem_.get_pdp_state(lteConfig.cid, regInfo.pdp_active); // update PDP state in info
        // struct after applying it to modem

        TelitCpsmsStatus psm_state;
        auto status = modem_.get_telit_psm(
            psm_state); // update PSM config in info struct after applying it to modem
        if (status == ModemStatus::ok) {
            /*
    ...
    case NetworkLteState::network_detached:
        NETWORK_LOG_INF(
            "State changed to network_detached, resetting network info and server states");
        ...
        call_action(
            ModemAction::attach_network); // trigger attach flow to try to re-attach to network
        break;
    case NetworkLteState::pdp_context_closed:
        NETWORK_LOG_INF(
            "State changed to pdp_context_closed, resetting IP address and server states");
        ...
        call_action(ModemAction::open_pdp_context); // trigger PDP activation flow to try to open
                                                    // PDP context and get an IP address
        break;
    }
}
...
// replace all occurrences of event_ = with on_event(event) to ensure that all events go through the
// on_event handler for better tracking and debugging
...
NetworkLteEvent NetworkLte::get_event() {
    ...
    event_ = NetworkLteEvent::none; // reset event after reading it to avoid processing the same
                                    // event multiple times, if we want to keep a history of events,
                                    // we can store them in a vector instead of resetting it
...
ModemAction NetworkLte::get_action() {
    ...
    modem_action_ = ModemAction::none; // reset action after reading it, since actions are one-time
                                       // triggers that should be executed once and then cleared
                                       // until the next time they are triggered
    return action;
}
...
static const char* state_to_str(NetworkLteState s) {
    ...
    case NetworkLteState::network_attaching:
        return "network_attaching";
    case NetworkLteState::pdp_context_closed:
        return "pdp_context_closed";
    case NetworkLteState::pdp_context_opening:
        return "pdp_context_opening";
    case NetworkLteState::data_ready:
        return "data_ready";
...
static const char* event_to_str(NetworkLteEvent e) {
    ...
    case NetworkLteEvent::context_rejected:
        return "context_rejected";
    case NetworkLteEvent::data_available:
        return "data_available";
    // Action completion events (generated by step())
    case NetworkLteEvent::attach_started:
        return "attach_started";
    case NetworkLteEvent::pdp_opening:
        return "pdp_opening";
...
static const char* modem_event_to_str(NetworkLteEvent e) {
    ...
    case NetworkLteEvent::data_available:
        return "data_available";
    default:
        return "unknown_modem_event";
    }
}

static const char* action_to_str(ModemAction a) {
    switch (a) {
    case ModemAction::none:
        return "none";
    // recovering actions
    case ModemAction::reboot:
        return "reboot";
    ...
    case ModemAction::power_off:
        return "power_off";
    case ModemAction::turn_on_radio:
        return "turn_on_radio";
    case ModemAction::switch_off_radio:
        return "switch_off_radio";
    case ModemAction::enter_sleep:
        return "enter_sleep";
    case ModemAction::wake_up:
        return "wake_up";
    ...
    case ModemAction::query_network_status:
        return "query_network_status";
    case ModemAction::query_pdp_context:
        return "query_pdp_context";
    // internal actions to drive state machine forwared
    case ModemAction::attach_network:
        return "attach_network";
    case ModemAction::open_pdp_context:
        return "open_pdp_context";
    // data actions
    case ModemAction::send_data:
        return "send_data";
    case ModemAction::read_data:
    ...
    // sepecial modes
    case ModemAction::enter_transparent_mode:
        return "enter_transparent_mode";
    case ModemAction::leave_transparent_mode:
        return "leave_transparent_mode";
    default:
...
void NetworkLte::log_action() const {
    if (modem_action_ != ModemAction::none)
        NETWORK_LOG_DBG("new action: %s", action_to_str(modem_action_));
}
...
bool NetworkLte::check_status_response(ModemStatus status) {
    if (status == ModemStatus::ok) {
        ...
        if (status == ModemStatus::timeout) {
            ...
            on_event(NetworkLteEvent::
                         at_command_no_response); // flag a generic network error for now, we can
                                                  // have more specific error events if needed (e.g.
                                                  // attach_error, context_error, etc.) and trigger
                                                  // them based on the current state and action that
