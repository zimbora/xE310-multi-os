...
void IpcServer::stop() {
    running_ = false;
    if (client_fd_ != -1) {
        closesocket(to_sock(client_fd_));
        client_fd_ = -1;
    }
    if (server_fd_ != -1) {
        closesocket(to_sock(server_fd_));
        server_fd_ = -1;
    }
    WSACleanup();
}
...
bool IpcServer::send(const uint8_t* data, uint16_t length) {
    ...
    if (mode_ == Mode::framed) {
        ...
        if (::send(c, reinterpret_cast<const char*>(hdr), 2, 0) != 2)
            return false;
        return ::send(c, reinterpret_cast<const char*>(data), length, 0) ==
