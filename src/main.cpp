...
int main(int argc, char* argv[]) {
    ...
    IpcServer ipc(
        9000, nullptr, IpcServer::Mode::line); // callback set after network is constructed
    ...
    at_ipc.set_connect_callback([&]() {
        ...
        network
            .enter_transparent_mode(); // this will internally call the modem action to enter
                                       // transparent mode, but we need to call execute_actions here
                                       // to actually perform the action and change the modem state
                                       // before we start receiving AT commands from the client
    });
    ...
    at_ipc.set_disconnect_callback([&]() {
        ...
        network
            .leave_transparent_mode(); // this will internally call the modem action to leave
                                       // transparent mode, but we need to call execute_actions here
                                       // to actually perform the action and change the modem state
                                       // before we can send AT commands in normal mode again
    ...
    auto handle_rpc = [&](const std::string& req) -> std::string {
        auto to_upper = [](std::string s) {
            std::transform(
                s.begin(), s.end(), s.begin(), [](unsigned char c) { return (char)toupper(c); });
            return s;
        };
        auto sp = req.find(' ');
        ...
        if (cmd == "GET") {
            ...
            if (sub == "MODEMINFO")
                return rpc::to_json(network.modem_info());
            if (sub == "SIMSTATUS")
                return "\"" + std::string(rpc::to_str(network.sim_status())) + "\"";
            if (sub == "RADIOTECH")
                return "\"" + std::string(rpc::to_str(network.radio_tech())) + "\"";
            if (sub == "REGSTATUS")
                return "\"" + std::string(rpc::to_str(network.reg_status())) + "\"";
            if (sub == "REGINFO")
                return rpc::to_json(network.registration_info());
            if (sub == "NETWORKINFO")
                return rpc::to_json(network.network_info());
            if (sub == "SIGNALQUALITY")
                return rpc::to_json(network.signal_quality());
            if (sub == "PSMMODE")
                return "\"" + std::string(rpc::to_str(network.psm_mode())) + "\"";
            if (sub == "CPSMSCONFIG")
                return rpc::to_json(network.cpsms_config());
            if (sub == "TELITCPSMSCONFIG")
                return rpc::to_json(network.telit_cpsms_config());
            if (sub == "TELITCPSMSSTATUS")
                return rpc::to_json(network.telit_cpsms_status());
            if (sub == "SURVEYRESULT")
                return rpc::to_json(network.network_survey_result());
            if (sub == "OPERATORLIST")
                return rpc::to_json(network.available_operators());
            if (sub == "SCANSURVEY") {
                network.scan_networks();
                return rpc::to_json(network.csurv_result());
            }
            if (sub == "STATE")
                return "\"" + std::string(rpc::to_str(network.state())) + "\"";
            if (sub.rfind("SERVERINFO", 0) == 0) {
                ...
                if (!arg.empty()) {
                    ...
                    return "ERROR: conn_id out of range (1-" +
                           std::to_string(MAX_SERVER_CONNECTIONS) + ")";
                }
                std::string r = "[";
        ...
        if (cmd == "SET") {
            ...
            std::string args = sp2 == std::string::npos
                                   ? ""
                                   : sub_orig.substr(sp2 + 1); // preserve original case for values
        ...
        return "ERROR: unknown command — use GET <resource>, SET CONFIG <key>=<value>, or SET "
               "NETWORKDISCONNECT [conn_id]";
    };
    ...
    if (res) {
        ...
        network.tx_write(
            lteConfig.conn_id, reinterpret_cast<const uint8_t*>(hello.data()), hello.size());
        network.call_action(modem::ModemAction::send_data);
    ...
    while (true) {
        ...
        while (network.rx_read(lteConfig.conn_id, rx_msg) == modem::QueueError::ok) {
            ...
            MODEM_LOG_INF("RX queue [conn %d]: %s (%zu bytes)",
                          lteConfig.conn_id,
                          payload.c_str(),
                          rx_msg.data.size());
        ...
        if(count%5==0){
            ...
            network.tx_write(lteConfig.conn_id, reinterpret_cast<const uint8_t*>(msg.data()),
        msg.size()); MODEM_LOG_INF("Queued TX message: %s", msg.c_str());
