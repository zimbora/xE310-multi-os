...
enum class NetworkLteEvent : uint8_t {
    ...
    psm_exit,  // wake up from sleep mode (PSM), triggered by GPIO Wake0, go to data ready, wait for
               // CTS line
    ...
    timeout, // triggered when we time out waiting for an expected event (e.g. attach completion,
             // PDP activation, server connection, data transfer)

    // errors
    network_error, // generic error for network-related failures (e.g. registration failure,
                   // unexpected detach, etc.) that can trigger retries and fallback mechanisms
                   // before giving up and going to done state
    attach_error,  // specific error for attach failures (e.g. after max attach retries reached)
    context_error, // specific error for PDP context activation failures (e.g. after max PDP
                   // activation retries reached)
    at_command_no_response, // error for unexpected AT command failures (e.g. modem becomes
                            // unresponsive, or returns an error to a command we expect to always
                            // succeed like AT or AT+CGMM). This can trigger recovery actions like
                            // modem reboot or factory reset.
};

/// Actions that the state machine can trigger on the modem. These are executed in loop() based on
...
enum class ModemAction : uint8_t {
    ...
    query_network_status, // triggered in idle mode to check if we are already registered to network
                          // or not, and decide whether to start attach flow or not
    query_pdp_context, // triggered in idle mode after network attach to check if we already have an
                       // active PDP context or not, and decide whether to start PDP activation flow
                       // or not
    ...
    // data actions
    send_data, // trigger data send flow (if in data ready state) or buffer data to send when we get
               // to data ready state
    read_data, // trigger data read flow (if in data ready state)
    data_complete, // triggered when we receive a solicitation from user that data transfer is
                   // complete and we can go back to idle or sleep mode, depending on the
                   // configuration
    // special modes
...
struct NetworkLteConfig {
    ...
    uint16_t transparent_timeout_sec =
        300; ///< Timeout for transparent mode in seconds, adjust as needed based on expected time
             ///< to send AT commands and receive responses in transparent mode
    ...
    std::string plmn = DEFAULT_PLMN; ///< Optional PLMN to attach to (e.g. "26801" for VDF PT). If
                                     ///< empty, modem default will be used.
    ...
    uint8_t conn_id =
        1; ///< Connection ID to query for server connection status (e.g. for UDP sockets)
};
...
struct ServerInfo {
    ...
    std::string address;
    uint16_t port = 0;
    bool fHasData = false; // flag to indicate if we have data to send to the server, used to
                           // trigger data send flow when we get to data ready state
};
...
public:
    using DataReceivedCallback =
        std::function<void(uint8_t cid, const std::string& data, uint16_t n_bytes)>;

    explicit NetworkLte(xE310& modem,
    ...
    /// Configure a new server connection with the given parameters. Does not trigger any state
    /// changes by itself.
    void new_connection(uint8_t conn_id,
                        const std::string& protocol,
                        const std::string& ip,
                        const std::string& port);
    /// Connect to the network and server using the current configuration. Blocks until connected or
    /// max retries reached.
    bool server_connect(uint8_t conn_id,
                        const std::string& protocol,
                        const std::string& ip,
                        const uint16_t port);
    ...
    /// Try to register on a network with PSM enabled, by checking the current network registration
    /// and PSM configuration, and iterating through available operators if registration or PSM is
    /// not available with the current one. Returns true if successfully registered on a network
    /// with PSM enabled, false otherwise.
    bool force_PSM();
    ...
    // !! private functions but set as public for testing purposes, since we want to be able to call
    // them directly from unit tests to test specific state transitions and flows without having to
    // go through the whole state machine loop and trigger the corresponding events

    // callback to retrieve data received from the modem (e.g. from +RECEIVE URC) and pass it to the
    // user-provided callback for processing
    ...
    // Dequeue the current event and return it. The event is reset to none after being read to avoid
    ...
    // Set the current action to be executed in the next step() call based on the current state and
    // event.
    void call_action(ModemAction action);

    // Get the current action and reset it to none. The action is reset after being read to avoid
    // executing the same action multiple times.
    ModemAction get_action();

    void execute_actions();

    // Force a transition to the given state by generating the necessary sequence of events and
    // calling loop() until the target state is reached.
...
private:
    ...
    uint8_t nNetworkAttempts =
        0;                      // number of consecutive network attempts (attach + PDP activation)
    ...
    bool fColdBoot = false; ///< Whether the modem has completed its first boot sequence since
                            ///< power-on, cold boot.
    bool fWarmBoot = true;  ///< Whether the modem has completed a subsequent boot sequence since
                            ///< power-on, warm boot (e.g. after a reset or firmware update).
    bool fNewConfig =
        false; ///< Whether the modem configuration has been changed and not yet applied.
    bool fChangeBands =
        false; ///< Whether to change bands after a failed attach attempt (for testing purposes,
               ///< since in some networks changing bands can help with attach success)
    bool fChangeRAT =
        false; ///< Whether to change RAT after a failed attach attempt (for testing purposes, since
               ///< in some networks changing RAT can help with attach success)
